...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
        ...
        try {
            Document movieById = moviesCollection.find(new Document("_id", new ObjectId(movieId))).first();
        ...
        List<Document> pipeline = Arrays.asList(new Document("$match",
                ...
                new Document("$lookup",
                        new Document("from", "comments")
                                ...
                                .append("pipeline", Arrays.asList(new Document("$sort", new Document("date", -1))))
                                .append("as", "comments")));
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        Bson queryFilter = new Document("countries",
                new Document("$all", Arrays.asList(country)));
