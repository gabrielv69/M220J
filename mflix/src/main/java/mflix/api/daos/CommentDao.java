...
public class CommentDao extends AbstractMFlixDao {
    ...
    public Comment addComment(Comment comment) {
        ...
        if(comment.getId()==null) {
            throw new IncorrectDaoOperation("Comment can not be empty or null.", new Throwable());
        }
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        Bson textChange = new Document("text", text);
        Bson setDocument = new Document("$set", textChange);
        UpdateResult resultUpdate = commentCollection.updateOne(queryFilter,setDocument);//, new UpdateOptions().upsert(false));
        // TODO> Ticket - Handling Errors: Implement a try catch block to
        // handle a potential write exception when given a wrong commentId.
    ...
    public boolean deleteComment(String commentId, String email) {
        ...
        // comment
        Bson queryFilter = Filters.and(Filters.eq("_id", new ObjectId(commentId)),Filters.eq("email",email));
        DeleteResult deleteResult= commentCollection.deleteOne(queryFilter);
